...
try {
  ...
  let playlists=Array.from({length:20000},(_,i)=>({id:i+1,name:i===0?'Local crate':`Crate ${i+1}`,track_count:0,source:'plumdeck',editable:true}));
  ...
  await page.route('**/api/**',async route=>{
    ...
    else if(path==='/api/play/playlists'&&method==='POST') {const body=route.request().postDataJSON();result={id:60001,name:body.name,track_count:0,source:'plumdeck',editable:true};playlists.unshift(result);}
    ...
    else if(path==='/api/play/rekordbox/rb/playlists/mix/copy') {const playlist={id:60002,name:'Copied mirror',track_count:1,source:'plumdeck',editable:true};playlists.unshift(playlist);result={playlist,copied:1,skipped_unresolved:1};}
  ...
  await page.screenshot({path:'/tmp/plumdeck-playlist-create-dialog.png',animations:'disabled'});
  ...
  await page.getByRole('button',{name:'plumdeckへコピー',exact:true}).click();
  ...
  await page.screenshot({path:'/tmp/plumdeck-library-pagination.png'});
