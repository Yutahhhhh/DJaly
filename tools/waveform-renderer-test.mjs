...
try{
 ...
 const result=await page.evaluate(async()=>{
  ...
  times.sort((a,b)=>a-b);return {stableUploads,gpuPixels,cpuPixels,p95:times[114],p99:times[118],diagnostics:r.diagnostics};
 });
 assert(result.stableUploads,'steady scrolling must reuse geometry');assert(result.gpuPixels>10000&&result.cpuPixels>10000,'both renderers must produce visible waveforms');assert(result.diagnostics.gpuBytes<=32*1024*1024);
