...
class WaveformRenderer {
  ...
  private geometry=new Map<WaveformTile,Geometry>();
  private bytes=0;
  readonly diagnostics={backend:'canvas2d',fallbacks:0,draws:0,uploads:0,gpuBytes:0,cpuMs:0};
  private initialize(){
    ...
    try{
      ...
      canvas.addEventListener('webglcontextlost',event=>{event.preventDefault();this.geometry.clear();this.bytes=0;this.gl=null;this.diagnostics.fallbacks++;});
  ...
  private upload(tile:WaveformTile){
    const old=this.geometry.get(tile);if(old){old.used=performance.now();return old;}
    ...
    const item={buffer,count:vertices.length/5,bytes:data.byteLength,used:performance.now()};this.geometry.set(tile,item);this.bytes+=item.bytes;this.diagnostics.uploads++;this.diagnostics.gpuBytes=this.bytes;return item;
  }
  draw(ctx:CanvasRenderingContext2D,tiles:WaveformTile[],startMs:number,spanMs:number,width:number,height:number,vertical:boolean,side:'left'|'right'){
    ...
    if(gl&&canvas&&program&&!this.forceCanvas&&!tiles.some(tile=>tile.framesPerBin===1)){
      try{
        const dpr=Math.min(2,window.devicePixelRatio||1),w=Math.min(8192,Math.round(width*dpr)),h=Math.min(8192,Math.round(height*dpr));
        if(w*h*4>32*1024*1024)throw new Error('raster budget');
        ...
        const viewport=gl.getUniformLocation(program,'viewport'),orientation=gl.getUniformLocation(program,'orientation');
        gl.enableVertexAttribArray(position);gl.enableVertexAttribArray(color);
        for(const tile of tiles){
          const item=this.upload(tile),spanFrames=spanMs*tile.sampleRate/1000;
          ...
          gl.uniform4f(viewport,(tile.startFrame-startMs*tile.sampleRate/1000)/spanFrames,tile.framesPerBin*2048/spanFrames,(breadth-12)/breadth*(vertical?1:.5),side==='left'?1-2/width:2/width);
        ...
        ctx.drawImage(canvas,0,0,width,height);this.diagnostics.draws++;this.diagnostics.cpuMs=performance.now()-start;return;
