...
export type ImportBatch = { id: string; request_id: string; state: string; total_items: number; succeeded_items: number; failed_items: number; skipped_items: number; target_kind: string; target_id?: number; items?: Array<Record<string, unknown>> };
...
export const workflowsService = {
  ...
  createUsbHandoff: (setlistId: number) => apiClient.post<UsbExport>("/workflows/usb/handoffs", { setlist_id: setlistId }),
  ...
  imports: (active = false) => apiClient.get<ImportBatch[]>("/workflows/play/imports", { active }),
