...
export function FilterDialog({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (enabledSections.includes("vibe")) fetchPresets();
      if (enabledSections.includes("metadata")) fetchGenres();
    }
  }, [isOpen, currentFilters, currentPreset, enabledSections]);
        ...
        <div className="grid gap-4 py-4">
          ...
          {localPreset === "custom" && (
            <div className="grid gap-6 px-1 animate-in fade-in slide-in-from-top-2 duration-300">
                ...
                <>
                  ...
                  <div className="space-y-3">
                    ...
                    <Slider
                      defaultValue={[0, 1]}
                      value={[
                        localFilters.minBrightness,
                        localFilters.maxBrightness,
                      ]}
                      max={1}
                      step={0.05}
                      minStepsBetweenThumbs={0.1}
              ...
              {/* Metadata Section */}
              {enabledSections.includes("metadata") && (
                <>
                  {/* Lyrics Search */}
                  <div className="space-y-2">
                    <Label className="flex items-center gap-2 text-xs font-semibold">
                      <Mic2 className="h-3.5 w-3.5 text-red-500" />
                      Lyrics
                    </Label>
                    <Input
                      placeholder="Search lyrics..."
                      className="h-8 text-xs"
                      value={localFilters.lyrics || ""}
                      onChange={(e) =>
                        setLocalFilters({
                          ...localFilters,
