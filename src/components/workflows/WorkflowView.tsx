...
function MediaPanel({ run }: { run: Runner }) {
  ...
  return <Shell icon={<FolderSync />} title="音源参照の診断と修復" description="まず診断し、候補を確認してから参照だけを更新します。曲ID・解析・履歴は維持されます。">
    <div className="grid gap-2 md:grid-cols-3"><Input value={ids} onChange={(e) => setIds(e.target.value)} placeholder="曲ID（空欄で全曲）"/><Input value={oldRoot} onChange={(e) => setOldRoot(e.target.value)} placeholder="以前の音源フォルダーの絶対パス"/><Input value={newRoot} onChange={(e) => setNewRoot(e.target.value)} placeholder="新しい音源フォルダーの絶対パス"/></div>
