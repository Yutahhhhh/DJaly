  ...
  return (
    <details id="junction-network-settings" className="junction-section" onToggle={(event) => setExpanded(event.currentTarget.open)}>
      ...
      <p className="junction-card-note">
        通常は設定不要です。招待・返答の文字はどのサーバーも経由しません。音声・データの接続先探索には既定で公開STUNを使います。音声・データが直接つながらないときだけ、セッション管理者が用意した中継（TURN）を追加できます。回線によっては接続できない場合もあります。設定は新しく作る招待から反映されます。
      </p>
