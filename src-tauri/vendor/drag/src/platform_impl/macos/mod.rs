...
define_class!(
    ...
    unsafe impl NSDraggingSource for DragRsSource {
        ...
        unsafe fn dragging_session(
            ...
            // JUCE destinations (including rekordbox) accept external files with
            // Generic rather than Copy. Advertise both without enabling Move.
            let operation: objc2_app_kit::NSDragOperation = ivars.drag_mode.into();
            let offered = if context == NSDraggingContext::OutsideApplication
                && matches!(ivars.drag_mode, DragMode::Copy)
            {
                operation | objc2_app_kit::NSDragOperation::Generic
            } else {
                operation
            };
            offered
