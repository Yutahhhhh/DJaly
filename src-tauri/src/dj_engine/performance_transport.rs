...
pub struct Transport {stream:UnixStream,input:Vec<u8>,generations:[u32;4],generation_since:[Instant;4],origin:Instant,offset:f64,seq:u64,last_write:Instant,pub observation:Observation}
impl Transport {
 pub fn connect(config:&Config)->Result<Self,String>{
  ...
  let mut transport=Self{stream,input:Vec::new(),generations:[0;4],generation_since:[origin;4],origin,offset,seq:0,last_write:Instant::now(),observation:Observation{active:true,cues:config.cues,ranges:config.ranges}};
  transport.observe(&state);Ok(transport)
 }
 fn observe(&mut self,value:&Value){for i in 0..4 {if let Some(g)=value["decks"][i]["generation"].as_u64(){if self.generations[i]!=g as u32 {self.generations[i]=g as u32;self.generation_since[i]=Instant::now();}}if let Some(c)=value["decks"][i]["cueMs"].as_f64(){self.observation.cues[i]=c;}if let Some(r)=value["decks"][i]["range"].as_f64(){self.observation.ranges[i]=r;}}}
 ...
 pub fn send(&mut self,bytes:&[u8],captured:Instant)->Result<(),String>{
  ...
  for fragment in 0..fragments {
   ...
   for i in 0..4{
    ...
    let generation=if captured>=self.generation_since[i]{self.generations[i]}else{0};
...
mod tests {
 ...
 fn queued_midi_is_not_relabelled_after_track_replacement(){
  ...
  let mut transport=Transport{stream,input:Vec::new(),generations:[1;4],generation_since:[origin;4],origin,offset:1e6,seq:0,last_write:origin,observation:Observation::default()};
  ...
  let mut record=[0u8;48];peer.read_exact(&mut record).unwrap();
  assert_eq!(u32::from_le_bytes(record[16..20].try_into().unwrap()),0);
  assert_eq!(u32::from_le_bytes(record[20..24].try_into().unwrap()),1);
  transport.send(&[0x90,0x36,0],Instant::now()).unwrap();peer.read_exact(&mut record).unwrap();
  assert_eq!(u32::from_le_bytes(record[16..20].try_into().unwrap()),2);
