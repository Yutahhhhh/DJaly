...
# ログディレクトリの作成
# 環境変数 PLUMDECK_LOG_DIR が設定されていればそれを使用 (本番環境/server.py経由)
# 設定されていなければ、このファイルからの相対パスを使用 (開発環境)
if "PLUMDECK_LOG_DIR" in os.environ:
    LOG_DIR = os.environ["PLUMDECK_LOG_DIR"]
else:
...
def get_logger(name: str):
    ...
    if not logger.handlers:
        ...
        log_file = os.path.join(LOG_DIR, "plumdeck.log")
        try:
