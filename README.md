...
Settingsの「解析方法」では、自動（Auto）・軽量（Light）・詳細（Full）を選べます。Windowsの自動解析はまず詳細解析を行い、処理がタイムアウトした場合は軽量解析へ切り替えてプレイ可能な状態まで完了させます。軽量解析は音源の代表区間（最大30秒・11.025kHzモノラル）からNumPyのみでBPM・キーと基本的な音響情報を概算し、曲名などのメタデータを登録します。類似曲検索用の埋め込みや、負荷の高いジャンル・音色の詳細解析は省くため、必要になった曲は「詳細」で再解析すると結果を更新できます。
...
低スペックWindowsでは「軽量」を明示的に選んでください。「自動」は最初に詳細解析を試すため、軽量設定と同じ待ち時間にはなりません。軽量経路はlibrosa・Numba・AIモデルを読み込まず、JITコンパイルも行いません。BPM・キーは代表区間の概算で、全曲のビート位置は生成しません。詳細なビートグリッドや類似曲検索が必要な曲は「詳細」で再解析してください。
...
短い通信断は15秒間、同じ接続の復旧を待ちます。戻らなければ、管理DJがDJ一覧の同じ行で「再接続の招待を作る」を押し、参加DJが「あなたの接続」の「新しい招待を入力」で受け取ります。参加DJは新しい返答を送り、管理DJが同じ行で返答を確認して接続を許可します。古い招待・返答は使えません。セッションを作り直す必要はありません。他のDJの接続と演奏権は維持します。取り消し・拒否をオフラインの相手に知らせる場合は、署名付き通知をコピーして渡します。DJ名やアイコンだけでは本人確認できないため、招待と返答は相手を確認できる経路で受け渡してください。
