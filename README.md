...
軽量経路はlibrosa・Numba・TensorFlowを読み込まず、JITコンパイルを行いません。ONNX RuntimeはCPU・1スレッドで使用します。5秒以上30分以内の音源を対象とし、曲ごとのワーカーは60秒で打ち切って失敗を再試行可能にします。拍や特徴量が不足する結果を「解析済み」として登録しません。PR CIとWindows配布物の診断では、5分の音源を新規プロセスで解析し、全曲の拍・波形・実モデルの特徴量を確認します。
