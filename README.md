...
Settingsの「解析方法」では、自動（Auto）・軽量（Light）・詳細（Full）を選べます。Windowsの自動解析はまず詳細解析を行い、処理がタイムアウトした場合は軽量解析へ切り替えてプレイ可能な状態まで完了させます。軽量解析は音源の代表区間（最大90秒）からBPM・キーと基本的な音響情報を取得し、曲名などのメタデータを登録します。類似曲検索用の埋め込みや、負荷の高いジャンル・音色の詳細解析は省くため、必要になった曲は「詳細」で再解析すると結果を更新できます。
...
JCTは複数のDJが一つのセッションへ参加し、演奏順と共有デッキを引き継ぐための機能です。参加者は表示名ではなくDJ名で扱い、任意のアイコンと色を設定できます。全員に同じ「DJ一覧」を表示し、招待を作った時点では相手を招待中の枠として追加します。返答を取り込んで接続すると、その枠が実際のDJ情報と接続状態に置き換わります。各行では参加待ち・接続済み・プレイ中などの状態と通信品質を確認できます。

セッションを作成したDJが管理を担当しますが、最初にプレイする必要はありません。管理DJは一覧をドラッグして予定順を変更し、接続済みの任意のDJを最初のプレイ担当に選べます。セッション開始後も一覧から次のDJを選び、準備完了を確認して引き継ぎます。
...
招待・返答の文字交換に、運営のシグナリングサーバーやアカウント、課金認証は不要です。管理DJが「セッションを作成」し、DJごとに招待用の文字を作ります。参加DJは招待を確認して返答を作り、管理DJがその返答をDJ一覧の該当行へ取り込んで接続を許可します。招待・返答はコピーまたはファイルで直接受け渡します。コピーしただけでは送信されないため、普段のチャットなどで相手へ渡してください。
...
既定では公開STUNを音声・データの直接接続先の探索に使います。招待・返答の文字は中継しません。相手とのメディア接続だけが直接つながらない場合に限り、管理DJが「直接つながらないときの中継設定」に自分のTURNを設定できます。「運営サーバー不要」は「外部サーバーを一切使わない」という意味ではなく、既定でも接続先探索用のSTUNは利用します。今回は運営クラウドを配備していません。
...
短い通信断は同じ接続の復旧を待ち、戻らなければDJ一覧の同じ行で新しい招待・返答を交換します。他のDJの接続と演奏権は維持します。取り消し・拒否をオフラインの相手に知らせる場合は、署名付き通知をコピーして渡します。DJ名やアイコンだけでは本人確認できないため、招待と返答は相手を確認できる経路で受け渡してください。
