...
bool AssetCache::begin(const QString& id,quint64 bytes,QString* error){
 ...
 int fd=::open(QFile::encodeName(d->path(id,".partial")).constData(),O_RDWR|O_CREAT|O_NOFOLLOW,0600);if(fd<0)return fail(error,"Cannot create partial asset");bool ok=ftruncate(fd,off_t(bytes))==0;::close(fd);if(!ok)return fail(error,"Cannot reserve asset file");d->transfers[id]=t;return d->save(id)||fail(error,"Cannot persist resume state");
}
bool AssetCache::put(const QString&id,quint64 offset,const QByteArray& chunk,QString* error){auto it=d->transfers.find(id);if(it==d->transfers.end())return fail(error,"Unknown transfer");auto&t=it->second;if(offset>=t.size||offset%kMaxChunkBytes||chunk.size()!=qint64(std::min<quint64>(kMaxChunkBytes,t.size-offset)))return fail(error,"Invalid chunk range");int fd=::open(QFile::encodeName(d->path(id,".partial")).constData(),O_RDWR|O_NOFOLLOW);if(fd<0)return fail(error,"Cannot open partial asset");auto slot=int(offset/kMaxChunkBytes);bool ok;if(t.bitmap[slot]){QByteArray old(chunk.size(),0);ok=pread(fd,old.data(),old.size(),off_t(offset))==old.size()&&old==chunk;}else{ok=pwrite(fd,chunk.constData(),chunk.size(),off_t(offset))==chunk.size()&&fsync(fd)==0;}::close(fd);if(!ok)return fail(error,"Conflicting or failed chunk");t.bitmap[slot]=1;return d->save(id)||fail(error,"Cannot persist chunk state");}
bool AssetCache::finalize(const QString&id,const std::function<bool(const QString&)>& decoder,QString* error){auto it=d->transfers.find(id);if(it==d->transfers.end()||it->second.bitmap.contains(char(0)))return fail(error,"Asset incomplete");auto path=d->path(id,".partial");if(!d->safe(path)||hashFile(path,error)!=id)return fail(error,"Asset hash mismatch");if(!decoder||!decoder(path))return fail(error,"Asset decoder validation failed");if(!d->safe(d->path(id)))return fail(error,"Unsafe asset destination");if(QFile::exists(d->path(id))){if(hashFile(d->path(id))!=id)return fail(error,"Existing cache integrity failure");QFile::remove(path);}else if(!QFile::rename(path,d->path(id)))return fail(error,"Atomic publish failed");QFile::remove(d->path(id,".resume"));d->transfers.erase(it);return true;}
