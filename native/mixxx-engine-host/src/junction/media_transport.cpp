...
struct MediaTransport::Impl {
 ...
 void attach(std::shared_ptr<rtc::Track> t){std::lock_guard attachLock(mutex);track=t;auto feedback=std::make_shared<FeedbackSession>();feedback->reportReceived=[this]{++receivedReports;};feedback->retry=[this](quint16 seq){std::lock_guard lock(mutex);auto i=resend.find(seq);if(i!=resend.end()&&i->second.retries<2&&monotonicNanos()-i->second.arrived<150000000&&track&&track->isOpen()){try{++i->second.retries;track->send(reinterpret_cast<const std::byte*>(i->second.bytes.constData()),i->second.bytes.size());++retransmittedPackets;}catch(...){}}};reportConfig=std::make_shared<rtc::RtpPacketizationConfig>(hasTx?tx.ssrc:1,"junction",111,48000);reporter=std::make_shared<rtc::RtcpSrReporter>(reportConfig);feedback->addToChain(reporter);t->setMediaHandler(feedback);t->onMessage([this](rtc::message_variant v){auto b=std::get_if<rtc::binary>(&v);if(!b||b->size()<12||b->size()>1500)return;auto p=reinterpret_cast<const uchar*>(b->data());if(p[0]!=0x80||(p[1]&127)!=111)return;std::lock_guard lock(mutex);if(!hasRx||qFromBigEndian<quint32>(p+8)!=rx.ssrc)return;quint32 ts=qFromBigEndian<quint32>(p+4);quint64 reference=rxStarted?nextRx:rx.mediaFrameOrigin;quint32 expected=rx.rtpTimestampOrigin+quint32(reference-rx.mediaFrameOrigin);qint64 delta=qint32(ts-expected);if(delta < -qint64(24000)||delta>qint64(96000)|| (delta<0&&quint64(-delta)>reference))return;quint64 frame=delta<0?reference-quint64(-delta):reference+quint64(delta);if(frame<rx.mediaFrameOrigin||(frame-rx.mediaFrameOrigin)%960||packets.size()>=100)return;++receivedPackets;auto seq=qFromBigEndian<quint16>(p+2);if(!receivedSequence||frame>lastReceivedFrame){lastReceivedFrame=frame;lastReceivedSequence=seq;receivedSequence=true;}packets.emplace(frame,Packet{QByteArray(reinterpret_cast<const char*>(p+12),b->size()-12),monotonicNanos()});});}
#endif
