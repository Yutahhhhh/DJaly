...
template<int Mode> class DdjBeatProcessor : public EffectProcessorImpl<DdjBeatState> {
public:
    ...
    void processChannel(DdjBeatState* s, const CSAMPLE* in, CSAMPLE* out,
            const mixxx::EngineParameters& p, EffectEnableState enabled, const GroupFeatureState& features) override {
        const double rate=p.sampleRate(), beats=beats_->value(), depth=depth_->value();
