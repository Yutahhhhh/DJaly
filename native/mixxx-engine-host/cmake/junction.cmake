# Djaly Junction native subsystem.
#
...
# is compiled into `djaly-junction-core` so it can be unit tested without an
...
if(JUNCTION_OPUS_INCLUDE AND JUNCTION_OPUS_LIBRARY)
  target_include_directories(djaly-junction-core PUBLIC "${JUNCTION_OPUS_INCLUDE}")
  target_link_libraries(djaly-junction-core PUBLIC "${JUNCTION_OPUS_LIBRARY}")
  target_compile_definitions(djaly-junction-core PUBLIC DJALY_JUNCTION_WITH_OPUS=1)
...
if(EXISTS "${JUNCTION_LDC_ROOT}/build-static/libdatachannel.a")
  target_include_directories(djaly-junction-core PUBLIC "${JUNCTION_LDC_ROOT}/include")
  target_link_libraries(djaly-junction-core PUBLIC
  ...
  foreach(ice_library nice gio-2.0 gobject-2.0 glib-2.0)
    ...
    target_link_libraries(djaly-junction-core PUBLIC ${JUNCTION_${ice_library}_LIBRARY})
  endforeach()
  find_package(OpenSSL REQUIRED)
  target_link_libraries(djaly-junction-core PUBLIC OpenSSL::SSL OpenSSL::Crypto)
  target_compile_definitions(djaly-junction-core PUBLIC DJALY_JUNCTION_WITH_LIBDATACHANNEL=1 RTC_STATIC=1)
...
target_include_directories(djaly-junction-core PUBLIC ${JUNCTION_AUDIO_INCLUDE_DIRS})
...
foreach(audio_library portaudio samplerate sndfile)
  ...
  target_link_libraries(djaly-junction-core PUBLIC ${JUNCTION_${audio_library}_LIBRARY})
endforeach()
...
target_link_libraries(djaly-junction-core PUBLIC djaly-soundtouch-state)
include("${CMAKE_CURRENT_LIST_DIR}/rubberband-checkpoint.cmake")
target_link_libraries(djaly-junction-core PUBLIC djaly-rubberband-state)
