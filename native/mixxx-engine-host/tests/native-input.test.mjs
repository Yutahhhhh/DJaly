...
test('native performance channel owns MIDI edges, faders, release and disconnect without a WebView',{timeout:15000},async()=>{
 const dir=await mkdtemp(path.join(tmpdir(),'djaly-native-input-'));const sampleRate=44100,frames=sampleRate*10,wave=Buffer.alloc(44+frames*4);
 ...
 for(let f=0;f<frames;f++){const v=Math.round(8000*Math.sin(f*.1));wave.writeInt16LE(v,44+f*4);wave.writeInt16LE(v,46+f*4);}const file=path.join(dir,'tone.wav');await writeFile(file,wave);
