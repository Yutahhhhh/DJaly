...
const directory=await mkdtemp(path.join(tmpdir(),'plumdeck-real-native-'));
const binary=process.env.PLUMDECK_TEST_HOST||path.resolve(import.meta.dirname,'../build-upstream/plumdeck-mixxx-engine-host');
const child=spawn(binary,[],{env:{...process.env,PLUMDECK_MIXXX_OUTPUT_DEVICE:process.env.PLUMDECK_MIXXX_OUTPUT_DEVICE||'BlackHole 2ch',PLUMDECK_WAVEFORM_CACHE:path.join(directory,'cache'),PLUMDECK_MIXXX_RECORDING_DIR:directory}});
