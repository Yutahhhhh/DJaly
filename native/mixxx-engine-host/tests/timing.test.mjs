...
const baseline = process.env.DJALY_TIMING_BASELINE === '1';
test('audio callback timing: scratch release PCM and placement-only quantize including SYNC', { timeout: 60000 }, async () => {
  const directory = await mkdtemp(path.join(tmpdir(), 'djaly-timing-'));
  const rate = Number(process.env.DJALY_TIMING_SOURCE_RATE || 44100), frames = rate * 40, wave = Buffer.alloc(44 + frames * 4);
